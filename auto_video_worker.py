...
def ai_post_production(topic, subtopic, state_name):
    """Sử dụng Gemini AI để xử lý hậu kỳ nội dung."""
    prompt = f"""
...
def run_worker():
    print("🤖 Video Factory Worker started (Independent Mode)...")
    while True:
        tasks = get_tasks()
        if tasks:
            ...
            for task in tasks:
                ...
                cap, tags, thumb = ai_post_production(topic, subtopic, state)
                
                if cap and tags and thumb:
                    if update_task(t_id, cap, tags, thumb):
